(function ($) {
    ...
    ajaxReq.onreadystatechange = function () {
        var ajaxsearchresults = document.getElementById("ajaxsearchresults");
        if (this.readyState == 4 && this.status == 200) {
            var result = this.responseText;
            ajaxsearchresults.innerHTML = result;
        }
    };
    searchbox.onkeyup = function () {
        ...
        ajaxReq.open("GET", "search?search=" + searchbox.value, true);
        ajaxReq.send();
    };
...
$('#change-icon').click(function () {
    ...
    iconInput.siblings('.demo-icon').attr('class', 'demo-icon btn btn-block');
...
function userlogin() {
    ...
    var usernameInfo = document.getElementById('span_loginid');
    var passwordInfo = document.getElementById('span_loginpsw');
    var isValidLogin = false;

    if (!username) {
        usernameInfo.innerHTML = 'Please enter username';
    ...
    if (!password) {
        passwordInfo.innerHTML = 'Please enter password';
...
function userupdate() {
    ...
    if (((password && !repassword) || (!password && repassword)) || ((password || repassword) && password !== repassword)) {
        formerror.innerHTML = 'Confirm Password does not match, please try again';
